...
//!
//! This library crate holds the shared core used by both shim variants:
//! - `hok-shim` (console subsystem — console targets; the invoking shell
//!   waits for the shim, so interactive children keep working)
//! - `hok-shim-gui` (GUI subsystem — GUI targets; no console window on
//!   double-click, and the shell does not wait, like running the GUI
//!   program directly)
//!
//! The PE entry points and the panic handler live in the bin targets.
...
// Duplicate with CRT symbols — resolved via /ALTERNATENAME in build.rs.
// Uniquely named (shim_*), so exporting them under `cargo test` too is
// harmless and avoids the lib test harness's memcpy/memset references
// being redirected to missing symbols.

// MSVC C++ exception-handling frame function, referenced by the prebuilt
// `core` rlib's `.xdata` EH tables. Never invoked — the shims build with
// `panic = "abort"` — but the symbol must resolve at link time. Not needed
// under `cargo test` (std links the real vcruntime symbol).
...
impl U16Buf {
    ...
    fn from_slice(s: &[u16]) -> Self {
        ...
        let mut b = Self::new();
        unsafe {
            core::ptr::copy_nonoverlapping(s.as_ptr(), b.mut_ptr(), n);
        }
        b.buf[n] = 0;
        b.len = n;
        b
...
/// Write to stderr. Used by the bin targets' panic handlers.
///
/// # Safety
///
/// `msg` must point to `msg.len()` readable bytes.
pub unsafe fn write_stderr(msg: &[u8]) {
...
unsafe fn skip_argv0(cmd: *mut u16) -> U16Buf {
    ...
    if *cmd.add(p) == b'"' as u16 {
        p += 1;
        while *cmd.add(p) != 0 && *cmd.add(p) != b'"' as u16 {
            p += 1;
        }
        if *cmd.add(p) == b'"' as u16 {
            p += 1;
...
pub(crate) fn is_known_field(key: &[u8]) -> bool {
    u16_ieq(key, b"path")
        || u16_ieq(key, b"args")
        || u16_ieq(key, b"cwd")
        || u16_ieq(key, b"workdir")
        || u16_ieq(key, b"elevate")
        || u16_ieq(key, b"runas")
}
...
unsafe fn read_shim_file(path: &U16Buf) -> ([u8; FILE_BUF], usize) {
    ...
    if h == INVALID_HANDLE || h == NULL_HANDLE {
        return (buf, 0);
    }
    let mut read: u32 = 0;
    ReadFile(
...
unsafe fn expand_env(src: &U16Buf) -> U16Buf {
    ...
    if required == 0 || required as usize > MAX_U16 {
        return *src;
    }
...
unsafe fn get_exe_stem() -> U16Buf {
    ...
    for i in (file_start..len as usize).rev() {
        if buf[i] == b'.' as u16 {
            ext_end = i;
            break;
        }
...
unsafe fn resolve_target(raw: &U16Buf, shim_dir: &U16Buf) -> U16Buf {
    ...
    if &raw_slice[..5] == prefix {
        let mut buf = [0u16; MAX_U16];
        let mut pos = 0;
        for &c in shim_dir.slice().iter() {
            if pos < MAX_U16 - 3 {
                buf[pos] = c;
                pos += 1;
            }
        }
        for &c in [b'\\' as u16, b'.' as u16, b'.' as u16, b'\\' as u16].iter() {
            buf[pos] = c;
            pos += 1;
        }
        for &c in raw_slice[5..].iter() {
            if pos < MAX_U16 - 2 {
                buf[pos] = c;
                pos += 1;
            }
        ...
        buf[pos] = 0;
        return U16Buf::from_slice(&buf[..pos]);
    }
    ...
    if len > 0 && (len as usize) < MAX_U16 {
        return u16_to_buf(resolved.as_ptr());
    }
    *raw
}
...
unsafe fn is_gui_app(path: &U16Buf) -> bool {
    ...
    // PE offset at 0x3C; guard against an offset that would overrun the
    // bytes actually read (no subtraction that could underflow)
    let pe_off = *(buf.as_ptr().add(0x3C) as *const u32) as usize;
    if pe_off as u32 + 0x5E > read {
        return false;
    }
    // Check PE signature
    if buf[pe_off] != b'P' || buf[pe_off + 1] != b'E' {
        return false;
    }
...
unsafe fn create_job() -> isize {
    ...
    if job == NULL_HANDLE || job == INVALID_HANDLE {
        return NULL_HANDLE;
    }
...
unsafe fn build_cmdline(
    path: &U16Buf,
    shim_args: &U16Buf,
    user_tail: &U16Buf,
    out: &mut [u16; MAX_U16 * 4],
) -> usize {
    let mut pos = 0usize;
    let max = MAX_U16 * 4 - 4;
...
unsafe fn do_elevate(path: &U16Buf, args: &U16Buf, user_tail: &U16Buf, cwd: &U16Buf) -> u32 {
    ...
    let mut sei = SEI {
        ...
        lpVerb: verb_buf.ptr(),
        lpFile: path.ptr(),
        lpParameters: cmdline.as_ptr(),
        lpDirectory: cwd_ptr,
        nShow: 1,
        hInstApp: 0,
        lpIDList: 0,
        lpClass: core::ptr::null(),
        hkeyClass: 0,
        dwHotKey: 0,
        hMonitor: 0,
        hProcess: 0,
    ...
    if proc_h == NULL_HANDLE || proc_h == INVALID_HANDLE {
        return 1;
    }
...
/// Run the shim: parse `.shim`, launch the target, forward its exit code.
///
/// # Safety
///
/// Relies on the Windows process environment; called once from the entry
/// points in the bin targets.
pub unsafe fn entry() -> i32 {
    ...
    if content_len == 0 {
        write_stderr(b"shim: cannot read .shim file\n");
        return 1;
    }
    ...
    if content_slice.len() >= 3
        && content_slice[0] == 0xEF
        && content_slice[1] == 0xBB
        && content_slice[2] == 0xBF
    {
    ...
    if !has_target {
        write_stderr(b"shim: no 'path' in .shim file\n");
        return 1;
    }
    ...
    if is_gui_app(&target_exp) {
        ...
        // Without this, since the GUI shim is a GUI binary, Windows
        // allocates a new console for the child (costly ~400ms).
        AttachConsole(ATTACH_PARENT_PROCESS);
    }
    ...
    if proc_h == NULL_HANDLE {
        return 1;
    }

    if job != NULL_HANDLE {
        AssignProcessToJobObject(job, proc_h);
    }
...
mod tests {
    use super::*;
    // `use std::*` alone does not bring prelude items (String, Vec) into a
    // `#![no_std]` crate's test module — the crate's prelude is core's.
    use std::prelude::v1::*;

    #[test]
    fn test_ieq_equal() {
        assert!(u16_ieq(b"hello", b"hello"));
    }
    #[test]
    fn test_ieq_case() {
        assert!(u16_ieq(b"PATH", b"path"));
    }
    #[test]
    fn test_ieq_mixed_case() {
        assert!(u16_ieq(b"RunAs", b"runas"));
    }
    #[test]
    fn test_ieq_diff_len() {
        assert!(!u16_ieq(b"path", b"paths"));
    }
    #[test]
    fn test_ieq_different() {
        assert!(!u16_ieq(b"path", b"args"));
    }

    #[test]
    fn test_known_path() {
        assert!(is_known_field(b"path"));
    }
    #[test]
    fn test_known_args() {
        assert!(is_known_field(b"args"));
    }
    #[test]
    fn test_known_cwd() {
        assert!(is_known_field(b"cwd"));
    }
    #[test]
    fn test_known_workdir() {
        assert!(is_known_field(b"workdir"));
    }
    #[test]
    fn test_known_elevate() {
        assert!(is_known_field(b"elevate"));
    }
    #[test]
    fn test_known_runas() {
        assert!(is_known_field(b"runas"));
    }
    #[test]
    fn test_known_case() {
        assert!(is_known_field(b"PATH"));
    ...
    #[test]
    fn test_known_unknown() {
        assert!(!is_known_field(b"custom"));
    }

    #[test]
    fn test_trim_none() {
        assert_eq!(trim_spaces(b"hello"), b"hello");
    }
    #[test]
    ...
    #[test]
    fn test_trim_cr() {
        assert_eq!(trim_spaces(b"hello\r"), b"hello");
    }
    #[test]
    fn test_trim_empty() {
    ...
    #[test]
    fn test_next_line_empty() {
        assert!(next_line(b"", 0).is_none());
    }
    ...
    fn test_parse_path() {
        ...
        assert_eq!(k, b"path");
        assert_eq!(v, b"C:\\test.exe");
    }
    #[test]
    fn test_parse_quoted() {
        let (k, v) = parse_key_value(b"path = \"C:\\Program Files\\app.exe\"").unwrap();
        assert_eq!(k, b"path");
        assert_eq!(v, b"C:\\Program Files\\app.exe");
    }
    #[test]
    fn test_parse_args() {
        let (k, v) = parse_key_value(b"args = --verbose --all").unwrap();
        assert_eq!(k, b"args");
        assert_eq!(v, b"--verbose --all");
    }
    #[test]
    fn test_parse_comment_hash() {
        assert!(parse_key_value(b"# this is a comment").is_none());
    }
    #[test]
    fn test_parse_comment_semicolon() {
        assert!(parse_key_value(b"; comment").is_none());
    }
    #[test]
    fn test_parse_comment_slash() {
        assert!(parse_key_value(b"// comment").is_none());
    }
    #[test]
    fn test_parse_empty() {
        assert!(parse_key_value(b"  ").is_none());
    }
    #[test]
    fn test_parse_no_equals() {
        assert!(parse_key_value(b"invalid").is_none());
    }
    #[test]
